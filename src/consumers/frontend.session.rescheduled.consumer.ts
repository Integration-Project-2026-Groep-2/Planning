...
export const startFrontendSessionRescheduledConsumer = async () => {
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      ...
      const data    = await parseXml(xml, 'FrontendSessionRescheduled');
      ...
      console.log('[Frontend] Sessie verzet via consumer:', session.sessionId);
      channel.ack(msg);
    } catch (err) {
      console.error('[Frontend] Fout in frontend.session.rescheduled:', err);
      await sendToDlq(
        xml,
