...
export const startLocationUpdatedConsumer = async () => {
  ...
  const queue    = 'planning.frontend.location.updated';
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      ...
      console.log('[Frontend] Locatie gewijzigd via consumer');
      channel.ack(msg);
    } catch (err) {
      console.error('[Frontend] Fout in frontend.location.updated:', err);
      await sendToDlq(
        xml,
